...
lib_dir := lib
tmp_dir := tmp
directories := $(bin_dir) $(lib_dir) $(tmp_dir) $(input_dir) $(output_dir)
...
informative_sites_per_chr_bed := $(addprefix $(tmp_dir)/, $(addprefix chr,$(addsuffix _freq_$(freq).bed,$(chromosomes))))
informative_sites_per_chr_vcf := $(addprefix $(tmp_dir)/, $(addprefix chr,$(addsuffix _freq_$(freq).vcf.gz,$(chromosomes))))
informative_sites_per_chr_tbi := $(addprefix $(tmp_dir)/, $(addprefix chr,$(addsuffix _freq_$(freq).vcf.gz.tbi,$(chromosomes))))

informative_sites_bed := $(output_dir)/nea_informative_sites_freq_$(freq).bed
informative_sites_vcf := $(output_dir)/nea_informative_sites_freq_$(freq).vcf.gz
informative_sites_tbi := $(output_dir)/nea_informative_sites_freq_$(freq).vcf.gz.tbi

raw_informative_sites_bed := $(tmp_dir)/raw_informative_sites_freq_$(freq).bed

.PHONY: clean scratch

.INTERMEDIATE: $(raw_informative_sites_freq) $(informative_sites_per_chr_bed)

default:
	@echo "Usage:"
	@echo -e "\tmake deps                 -- prepare depencies (binaries, directories etc.)"
	@echo
	@echo -e "\tmake scan [freq=0.0] -- scan the genome for archaic-like alleles allowing"
...
$(tmp_dir)/%.vcf.gz.tbi: $(tmp_dir)/%.vcf.gz
	tabix -f $<

$(tmp_dir)/chr%.vcf.gz: $(tmp_dir)/chr%.bed
	chr_id=$(subst chr,,$(subst _freq_$(freq).vcf.gz,,$(notdir $@))); \
...
$(raw_informative_sites_bed): $(informative_sites_per_chr_bed) $(informative_sites_vcf)
...
$(informative_sites_bed): $(raw_informative_sites_bed)
	cut -f1-6 $< > $@

$(tmp_dir)/chr%.bed: $(bin)
	chr_id=$(subst chr,,$(subst _freq_$(freq),,$(basename $(notdir $@)))); \
	hg1k_vcf_file="$(hg1k_vcf_path)/ALL.chr$${chr_id}.*.vcf.gz"; \
	...
	$(bin) $${chr_id} $${hg1k_vcf_file} $${altai_vcf_file} $${vindija_vcf_file} $(freq) > $@
